export class PolarGrid {
  constructor({ width, cx, cy }) {
    console.log(width, cx, cy)
    this._svgRadius = (width * 0.99) / 2;
    this._ringSpacing = ((width * 0.99) / 2) / 50;
    this._xAxisCenter = cx
    this._yAxisCenter = cy
    this._polarGridPath = this.createPolarGridPath();
  }

  get polarGridPath() { return this._polarGridPath };
  // Get the line starting x point
  xStartPoint = (radian, theta) => parseFloat((this._xAxisCenter + this._ringSpacing * radian * -Math.cos(theta)).toFixed(4));
  // Get the line starting y point
  yStartPoint = (radian, theta) => parseFloat((this._yAxisCenter + this._ringSpacing * radian * Math.sin(theta)).toFixed(4));
  // Get the line ending x point
  xEndPoint = (theta) => parseFloat((this._xAxisCenter + this._svgRadius * -Math.cos(theta)).toFixed(4));
  // Get the line ending y point
  yEndPoint = (theta) => parseFloat((this._yAxisCenter + this._ringSpacing * 50 * Math.sin(theta)).toFixed(4));
...
// Creates the rings of the polar grid
  createRings = () => {
    ...
    while (i < this._svgRadius + this._ringSpacing) {
      ...
      ringsPath = ringsPath + this.circlePathTemplate(this._xAxisCenter, this._yAxisCenter, radius);
...
// Create full lines at 30 degree increments
  createFullLines = () => {
    ...
    for (let i = 0; i < 360; i += 30) {
      ...
      fullRingsPath = fullRingsPath + `M ${this.xStartPoint(0.5, theta)}, ${this.yStartPoint(
        0.5,
        theta
  ...
  createLongLines = () => {
    ...
    for (let i = 0; i < 360; i += 10) {
      ...
      const theta = i * (Math.PI / 180);
      longLinesPath = longLinesPath + `M ${this.xStartPoint(5, theta)}, ${this.yStartPoint(
        5,
        theta
      )} L ${this.xEndPoint(theta)}, ${this.yEndPoint(theta)} `;
  ...
  createMediumLines = () => {
    ...
    for (let i = 0; i < 360; i += 2) {
      ...
      const theta = i * (Math.PI / 180);
      mediumLinesPath = mediumLinesPath + `M ${this.xStartPoint(15, theta)}, ${this.yStartPoint(
        15,
        theta
      )} L ${this.xEndPoint(theta)}, ${this.yEndPoint(theta)} `;
  ...
  createShortLines = () => {
    ...
    for (let i = 0; i < 360; i += 1) {
      ...
      const theta = i * (Math.PI / 180);
      shortLinesPath = shortLinesPath + `M ${this.xStartPoint(35, theta)}, ${this.yStartPoint(
        35,
        theta
      )} L ${this.xEndPoint(theta)}, ${this.yEndPoint(theta)} `;
  ...
  createPolarGridPath = () => this.circlePathTemplate(
      this._xAxisCenter,
      this._yAxisCenter,
